...
public class Week05OOPLab {
	...
	public static void main(String[] args) {
		...
		//			b. Getters & Setters 
		//
		System.out.println("Question 1: Card Class");
		// Add your code here to instantiate a Card
		
		Card testCard = new Card("Four", "Clubs", 4);
		testCard.describe();
		...
		Card isThisYourCard = tempDeck.draw();
		isThisYourCard.describe();
		...
		Map<String, List<Card>> game5 = createNewGame(4);
		for (int person = 1; person <= 4; person++) {
			String name = "Player " + person;
			List<Card> hand = game5.get(name);
			System.out.println("--------" + name + " has the following " + hand.size() + " cards: ");
			for (Card card : hand) {
				card.describe();
			}
		}
	...
	public static Map<String, List<Card>> createNewGame (int numPlayers) {
		Map<String, List<Card>> newGame = new HashMap<>();
		Deck newDeck = new Deck();
		newDeck.shuffle();
		System.out.println(52/numPlayers);
		for (int person = 1; person <= numPlayers; person++) {
			newGame.put("Player " + person, newDeck.draw(52/numPlayers));
		}
